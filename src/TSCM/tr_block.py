...
def MinkoskiConvBNReLU(inplanes, planes, kernel_size=1, device=None):
    if device is None:
        device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    return nn.Sequential(
            ME.MinkowskiConvolution(inplanes, planes, kernel_size=kernel_size, dimension=3).to(device),
            ME.MinkowskiReLU(),
            ).to(device)
...
class CodedVTRBlock(nn.Module): # ddp could not contain unused parameter, so donnot inherit from TRBlock
    ...
    def __init__(self,
        ...
        if self.inplanes != self.planes:
            self.linear_top = MinkoskiConvBNReLU(inplanes, planes, kernel_size=1, device=device)
            self.downsample = ME.MinkowskiConvolution(inplanes, planes, kernel_size=1, dimension=3).to(device)

        if self.conv_v == True:
            self.v = nn.Sequential(
                    MinkoskiConvBNReLU(planes, planes, kernel_size=3, device=device),
                    MinkoskiConvBNReLU(planes, planes*self.expansion, kernel_size=1, device=device),
                    ).to(device)
        else:
            self.v = MinkoskiConvBNReLU(planes, planes*self.expansion, kernel_size=1, device=device)
        ...
        if self.CUSTOM_KERNEL:  # distinct geometric shape for codebook elements
            ...
            if not self.skip_choice:
                if self.qk_type == 'conv':
                    ...
                    self.q = MinkoskiConvBNReLU(planes, self.M, kernel_size=1, device=device)
